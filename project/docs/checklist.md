...
- **Module path:** `github.com/dan/fincom/alert-service` — **placeholder**; actual remote is `github.com/dangolds/idoalerts` (surfaced by `git push`). Safe to rename pre-Phase 2 via `go mod edit -module <new>` (no source files yet = no import updates). Not doing so now unless team-lead says — no blocker.
