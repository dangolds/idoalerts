...
|---|---|---|
| [Alert Management](alert-management/overview.md) | In progress (domain layer done, ports + service + API + wiring pending) | Sanctions-screening alert lifecycle service: create, list with tenant/status/score filters, escalate (OPEN → ESCALATED), write-once decide (OPEN \| ESCALATED → CLEARED \| CONFIRMED_HIT). Multi-tenant, in-memory storage, stdout event bus. |
